...
const interviewReportController = async (req, res) => {
    try {
        ...
        return res.status(201).json({ message: "Interview Report Created", interviewReport });
    } catch (error) {
...
const getAllInterviewReports = async (req, res) => {
    try {
        const interviewReports = await InterviewReport.find({ user: req.user._id })
            .sort({ createdAt: -1 })
            .select("-selfDescription -jobDescription -resume -__v -technicalQuestions -behavioralQuestions -preparationPlan -skillGaps -updatedAt");
...
// Generate Resume PDF — GET /api/interview/resume/pdf/:interviewId
